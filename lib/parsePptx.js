...
export async function parsePptx(buffer) {
  ...
  const { colors: themeColors, fonts } = await extractTheme(zip, parser);

  // --- Background image + shapes: try slides 1–3, use first that yields something ---
  let backgroundImage = null;
  let templateShapes = [];
  const slideColorSet = new Set(themeColors.map(c => c.toUpperCase()));
  const slideColors = [...themeColors];
  ...
  for (let n = 1; n <= 3; n++) {
    ...
    // Collect fill colors from shapes on this slide
    for (const color of extractSlideColors(slideDoc, themeColors)) {
      const upper = color.toUpperCase();
      if (!slideColorSet.has(upper)) {
        slideColorSet.add(upper);
        slideColors.push(color);
      }
    }

    if (templateShapes.length === 0) {
      templateShapes = extractTemplateShapes(slideDoc, slideColors);
    }
  }

  return { colors: slideColors, fonts, backgroundImage, templateShapes };
}
...
// Collect all fill colors used in shapes on a slide
function extractSlideColors(slideDoc, themeColors) {
  const colors = [];
  try {
    const spTree = slideDoc?.['p:sld']?.['p:cSld']?.['p:spTree'];
    if (!spTree) return colors;

    let spList = spTree['p:sp'];
    if (!spList) return colors;
    if (!Array.isArray(spList)) spList = [spList];
    ...
    for (const sp of spList) {
      try {
        ...
        const schemeClr = solidFill['a:schemeClr'];
        if (schemeClr) {
          const resolved = resolveSchemeColor(schemeClr['@_val'], themeColors);
          if (resolved) colors.push(resolved);
        }
      } catch { /* skip malformed shape */ }
    }

    // Also check group shapes (p:grpSp)
    let grpList = spTree['p:grpSp'];
    if (grpList) {
      if (!Array.isArray(grpList)) grpList = [grpList];
      for (const grp of grpList) {
        let innerSp = grp['p:sp'];
        if (!innerSp) continue;
        if (!Array.isArray(innerSp)) innerSp = [innerSp];
        for (const sp of innerSp) {
          try {
            const solidFill = sp['p:spPr']?.['a:solidFill'];
            if (!solidFill) continue;
            const srgb = solidFill['a:srgbClr'];
            if (srgb?.['@_val']) { colors.push(`#${srgb['@_val']}`); continue; }
            const schemeClr = solidFill['a:schemeClr'];
            if (schemeClr) {
              const resolved = resolveSchemeColor(schemeClr['@_val'], themeColors);
              if (resolved) colors.push(resolved);
